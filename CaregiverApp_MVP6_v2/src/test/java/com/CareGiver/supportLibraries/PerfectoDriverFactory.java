...
public class PerfectoDriverFactory {
	...
	public static DesiredCapabilities getPerfectoExecutionCapabilities() {
		...
		byte[] valueDecoded = Base64.decodeBase64(mobileProperties.getProperty("PerfectoPassword"));
		;
		String strPassword = new String(valueDecoded);

		desiredCapabilities.setCapability("user", mobileProperties.getProperty("PerfectoUser"));

		desiredCapabilities.setCapability("password", strPassword);
		/*
	 ...
	 * 
	 * @param manufacturer
	 *            The manufacturer of the device to be used for the test execution
	 *            (Samsung, Apple, etc.)
	 * @param model
	...
	public static AppiumDriver getPerfectoAppiumDriver(SeleniumTestParameters testParameters) {
		...
		try {
			...
			case ANDROID:
				...
				System.out.println("Successfully added all desired capabilities!!");
				try {
					driver = new AndroidDriver(new URL(HostURL), desiredCapabilities);
